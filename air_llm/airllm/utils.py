...
def save_quant_state_to_dict(self, packed=True):
    ...
    non_tensor_dict = {
        k: v for k, v in qs_dict.items() if not isinstance(v, torch.Tensor)
    }
...
def compress_layer_state_dict(layer_state_dict, compression=None):
    ...
    if compression == "4bit":
        ...
        for k, v in layer_state_dict.items():
            ...
            for quant_state_k, quant_state_v in save_quant_state_to_dict(
                quant_state
            ).items():
                compressed_layer_state_dict[k + ".4bit." + quant_state_k] = (
                    quant_state_v
                )
    elif compression == "8bit":
        compressed_layer_state_dict = {}
    ...
    return (
        compressed_layer_state_dict
        if compressed_layer_state_dict is not None
        else layer_state_dict
    )
...
def split_and_save_layers(
    ...
    if os.path.exists(checkpoint_path / "pytorch_model.bin.index.json"):
        ...
        assert os.path.exists(checkpoint_path / "model.safetensors.index.json"), (
    ...
    if layer_names is None:
        n_layers = len(
            set([int(k.split(".")[2]) for k in index.keys() if "model.layers" in k])
        )
    else:
        n_layers = len(
            set(
    ...
    if os.path.exists(saving_path):
        ...
        for layer in layers:
            found_layers[layer] = (
                ModelPersister.get_model_persister().model_persist_exist(
                    layer, saving_path
                )
            )
...
def find_or_create_local_splitted_path(
    ...
    if os.path.exists(model_local_path_or_repo_id):
        if os.path.exists(
            ...
            Path(model_local_path_or_repo_id) / "model.safetensors.index.json"
        ):
